    ...
    <div className="font-[family-name:var(--font-mono)] text-sm">
      ...
      <div className="pl-4 space-y-px">
        {files.map((entry) => (
          ...
          >
            <span className="text-white/35 w-[82px] shrink-0">{getPermissions(entry.file.type)}</span>
            <span className="text-white/45 w-[62px] shrink-0 text-right mr-2">{formatBytes(entry.file.size)}</span>
            <span className="text-white/35 w-[96px] shrink-0">{dateStr}</span>
            <span className={`${getTypeColor(entry.file.type)} truncate flex-1`}>
              {entry.file.name}
            </span>
            <button
              onClick={() => onRemoveFile(entry.id)}
              className="w-4 h-4 flex items-center justify-center text-white/0 group-hover:text-white/30 hover:!text-danger text-[10px] cursor-pointer bg-transparent border-none transition-colors duration-150 shrink-0 ml-2"
      ...
      {/* Summary */}
      <div className="text-white/40 mt-1.5 pl-4">
