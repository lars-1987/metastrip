...
export function TerminalTabBar({
  ...
  return (
    <div className="flex items-center bg-white/[0.02] border-b border-white/[0.06] overflow-x-auto shrink-0 scrollbar-none">
          ...
          >
            ...
            {tabs.length > 1 && (
              <span
                ...
                className="w-4 h-4 flex items-center justify-center rounded text-[10px] text-white/30 hover:text-white/70 hover:bg-white/[0.08] opacity-0 group-hover:opacity-100 transition-all duration-150"
              >
                ×
              </span>
            )}
      ...
      <button
        onClick={onNewTab}
        className="flex items-center justify-center w-8 h-8 mx-1 text-white/30 hover:text-white/60 hover:bg-white/[0.04] rounded transition-colors duration-150 cursor-pointer bg-transparent border-none text-sm font-[family-name:var(--font-mono)] shrink-0"
        aria-label="New tab"
      >
        +
      </button>
