...
const JOKE_CONTENT: Record<string, { title: string; content: React.ReactNode; width?: number }> = {
  env: {
    ...
    width: 380,
    content: (
      <pre className="text-[12px] font-[family-name:var(--font-mono)] leading-relaxed text-white/70 whitespace-pre-wrap">
        <span className="text-white/30"># DO NOT COMMIT THIS FILE</span>{"\n"}
        <span className="text-white/30"># ...seriously, we mean it this time</span>{"\n\n"}
        ...
        <span className="text-white/30"># the only env vars you need when</span>{"\n"}
        <span className="text-white/30"># everything runs in the browser</span>
      </pre>
    ),
  },
  nodemodules: {
    ...
    content: (
      <div className="text-[12px] font-[family-name:var(--font-mono)] leading-snug">
        ...
        <div className="flex items-center gap-1.5 mb-3 pb-2 border-b border-white/[0.06] text-[11px]">
          <span className="text-white/30">~/metastrip/</span>
        ...
        <div className="space-y-0.5">
          {[
            ...
            { name: ".package-lock.json", size: "94 MB", icon: "📄", color: "text-white/50" },
          ].map((f) => (
            <div key={f.name} className="flex items-center gap-2 px-2 py-1 rounded hover:bg-white/[0.04] group cursor-default">
              ...
              <span className={`flex-1 ${f.color} group-hover:text-white/80 transition-colors truncate`}>{f.name}</span>
              <span className="text-white/20 text-[10px] shrink-0">{f.size}</span>
            </div>
          ))}
        </div>
        {/* Status bar */}
        <div className="flex items-center justify-between mt-3 pt-2 border-t border-white/[0.06] text-[10px] text-white/20">
      ...
      <div className="text-[12px] font-[family-name:var(--font-mono)] leading-snug">
        ...
        <div className="space-y-0.5">
          {[
            ...
            { name: "definitely_not_memes/", size: "—", icon: "📁", meta: "nice try", metaColor: "text-white/20" },
          ].map((f) => (
            <div key={f.name} className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-white/[0.04] group cursor-default">
              <span className="text-[13px]">{f.icon}</span>
              <div className="flex-1 min-w-0">
                <div className="text-white/70 group-hover:text-white/90 transition-colors truncate">{f.name}</div>
        ...
        <div className="flex items-center justify-between mt-3 pt-2 border-t border-white/[0.06] text-[10px]">
          <span className="text-white/20">7 items — 18.1 MB</span>
  ...
  photo: {
    ...
    content: (
      <div className="text-[12px] font-[family-name:var(--font-mono)] leading-relaxed">
        ...
        <div className="text-white/40 mb-2">EXIF Metadata:</div>
...
export function TerminalApp() {
  ...
  return (
    <>
      ...
      <div
        className="relative z-[1] flex items-start justify-center min-h-screen md:py-0 pointer-events-none"
        style={{
